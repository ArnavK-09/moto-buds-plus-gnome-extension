      ...
      if (gIcon && iconName)
        this.set_child(
          new St.Icon({
            ...
            icon_size: 26,
            style_class: "mbp-toggle-icon",
          }),
        );
      if (label) attachTooltip(this, label);
    }

    setActive(on) {
      if (on) this.add_style_pseudo_class("checked");
  ...
  class ToggleRow extends St.BoxLayout {
    _init(title, items, { columns = 4 } = {}) {
      super._init({
        vertical: true,
        x_expand: true,
        style_class: "mbp-toggle-row",
      });
      if (title) this.add_child(new SectionTitle(title));
      const rows = Math.ceil(items.length / columns);
      for (let r = 0; r < rows; r++) {
        const hbox = new St.BoxLayout({
          x_expand: true,
          style_class: "mbp-toggle-strip",
        });
        for (let c = 0; c < columns; c++) {
          const item = items[r * columns + c];
          if (item) hbox.add_child(item.button);
          else hbox.add_child(new St.Bin({ x_expand: true }));
        }
        this.add_child(hbox);
      }
    }
  ...
  class ToggleGrid extends St.BoxLayout {
    _init(pills) {
      super._init({
        vertical: true,
        x_expand: true,
        style_class: "mbp-pill-grid",
      });
      for (let i = 0; i < pills.length; i += 2) {
        const row = new St.BoxLayout({ x_expand: true });
        row.add_child(pills[i]);
        if (pills[i + 1]) row.add_child(pills[i + 1]);
        else row.add_child(new St.Bin({ x_expand: true }));
        this.add_child(row);
      }
    }
  },
...
export const BatteryRing = GObject.registerClass(
  ...
  class BatteryRing extends St.Widget {
    ...
    update(info) {
      ...
      if (low) this._icon.add_style_class_name("mbp-batt-inset-low");
      else this._icon.remove_style_class_name("mbp-batt-inset-low");
    }
  },
);
  ...
  class BatteryCell extends St.BoxLayout {
    _init(label, iconName, gIcon, info) {
      super._init({
        vertical: true,
        x_expand: true,
        style_class: "mbp-battery-card",
      });
      this._ring = new BatteryRing(iconName, gIcon);
      this._ring.x_align = Clutter.ActorAlign.CENTER;
      this._level = new St.Label({
        style_class: "mbp-battery-level",
        x_align: Clutter.ActorAlign.CENTER,
        x_expand: true,
      });
      this._name = new St.Label({
        text: label,
        style_class: "mbp-battery-name",
        x_align: Clutter.ActorAlign.CENTER,
        x_expand: true,
      });
