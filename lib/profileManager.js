...
export const ProfileManager = GObject.registerClass({
    ...
    async _registerProfile(uuid) {
        ...
        try {
            registrationId = this._systemBus.register_object(
                objectPath, this._iface, this._onMethodCall.bind(this), null, null);
        } catch (e) {
        ...
        let proxy;
        try {
            proxy = await Gio.DBusProxy.new(this._systemBus, Gio.DBusProxyFlags.NONE,
                null, 'org.bluez', '/org/bluez', 'org.bluez.ProfileManager1', null);
        } catch (e) {
        ...
        try {
            await proxy.call('RegisterProfile',
    ...
    _unregisterProfile() {
        ...
        if (!p)
            return;
        try {
            p.proxy.call_sync('UnregisterProfile',
    ...
    async _deviceProxy(devicePath) {
        try {
            return await Gio.DBusProxy.new(this._systemBus, Gio.DBusProxyFlags.NONE,
                null, 'org.bluez', devicePath, 'org.bluez.Device1', null);
        } catch (e) {
    ...
    async _connectProfile(devicePath) {
        ...
        if (!proxy)
            return;
        try {
            await proxy.call('ConnectProfile',
    ...
    async _disconnectProfile(devicePath) {
        ...
        if (!proxy)
            return;
        try {
            await proxy.call('DisconnectProfile',
                GLib.Variant.new_tuple([GLib.Variant.new('s', this._profile.uuid)]),
                Gio.DBusCallFlags.NONE, -1, null);
        } catch (e) {
    ...
    async releaseFd(devicePath, disconnect = true) {
        ...
        this._fdByDevice.delete(devicePath);
        if (disconnect)
            await this._disconnectProfile(devicePath);
        if (this._fdByDevice.size === 0)
