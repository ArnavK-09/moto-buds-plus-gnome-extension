...
export const PanelIndicator = GObject.registerClass({
    ...
    _init(settings, device, gIcon, onOpenPrefs, compact) {
        ...
        if (compact)
            this.add_style_class_name('mbp-tray');

        const box = new St.BoxLayout({style_class: 'mbp-panel-box', y_align: Clutter.ActorAlign.CENTER});
        this._icon = new St.Icon({
            gicon: gIcon('bbm-earbuds-symbolic.svg'),
            style_class: 'system-status-icon',
            y_align: Clutter.ActorAlign.CENTER,
        });
        this._label = new St.Label({y_align: Clutter.ActorAlign.CENTER, style_class: 'mbp-panel-label'});
        box.add_child(this._icon);
        box.add_child(this._label);
        this.add_child(box);

        this._popup = new DevicePopup(this.menu, device, settings, gIcon, onOpenPrefs, compact ? 'tray' : 'panel');
        if (device)
    ...
    _update() {
        if (!this._device || this._compact) {
            ...
            this._icon.opacity = this._device ? (this._device.state.connected ? 255 : 140) : 120;
            return;
        }
