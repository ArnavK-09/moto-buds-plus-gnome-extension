...
export const ToggleButton = GObject.registerClass({
    ...
    _init(label, value, gIcon, iconName) {
        ...
        if (gIcon && iconName)
            box.add_child(new St.Icon({gicon: gIcon(iconName), icon_size: 26, style_class: 'mbp-toggle-icon'}));
        box.add_child(new St.Label({text: label, x_align: Clutter.ActorAlign.CENTER,
            style_class: 'mbp-toggle-caption'}));
        this.set_child(box);
    }
...
export const TogglePill = GObject.registerClass({
    ...
    _init(label, iconName) {
        ...
        const box = new St.BoxLayout({style_class: 'mbp-pill-box', x_align: Clutter.ActorAlign.CENTER});
        const icon = new St.Icon({icon_name: iconName, icon_size: 15, style_class: 'mbp-pill-icon'});
