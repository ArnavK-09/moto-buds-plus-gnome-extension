...
export const PRESET_ORDER = [PRESET.DEFAULT, PRESET.HEAVY_BASS, PRESET.CUSTOM];
...
export function presetForBands(bands) {
  ...
  for (const [preset, presetBands] of Object.entries(PRESET_BANDS)) {
    if (JSON.stringify(rounded) === JSON.stringify(presetBands)) return preset;
  }
