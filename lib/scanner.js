...
export const DeviceScanner = GObject.registerClass({
    ...
    async init() {
        try {
            const raw = await this._bus.call(BLUEZ, '/', OBJ_MANAGER, 'GetManagedObjects',
            ...
            this._propId = this._bus.signal_subscribe(BLUEZ, PROPS, 'PropertiesChanged',
                null, DEVICE_IFACE, Gio.DBusSignalFlags.NONE, this._onProps.bind(this));
            this._addedId = this._bus.signal_subscribe(BLUEZ, OBJ_MANAGER, 'InterfacesAdded',
                null, null, Gio.DBusSignalFlags.NONE, this._onAdded.bind(this));
            this._removedId = this._bus.signal_subscribe(BLUEZ, OBJ_MANAGER, 'InterfacesRemoved',
                null, null, Gio.DBusSignalFlags.NONE, this._onRemoved.bind(this));
            this._rescan();
        } catch (e) {
    ...
    _onProps(_c, _s, path, _i, _sig, params) {
        ...
        if ('Alias' in changed)
            dev.alias = changed.Alias.deep_unpack();
        if ('UUIDs' in changed)
            dev.uuids = changed.UUIDs.deep_unpack();
    ...
    destroy() {
        if (this._propId)
            this._bus.signal_unsubscribe(this._propId);
        if (this._addedId)
            this._bus.signal_unsubscribe(this._addedId);
        if (this._removedId)
            this._bus.signal_unsubscribe(this._removedId);
        this._devices.clear();
        this._emitted.clear();
    }
});