...
export const MotoBudsSocket = GObject.registerClass(
  ...
  class MotoBudsSocket extends GObject.Object {
    ...
    _querySequence() {
      const ops = [
        OP.GET_BATTERY_LEVEL,
        OP.GET_ANC_MODE,
        OP.GET_TOGGLE_CONFIGS,
        OP.GET_IN_EAR_DETECTION,
        OP.GET_DUAL_CONNECTION,
        OP.GET_HI_RES_MODE,
        OP.GET_GAME_MODE,
        OP.GET_VOLUME_BOOST,
        OP.GET_PROFILE_VERSION,
      ];
      this._qOps = ops;
      this._qIdx = 0;
      if (this._qTimer) GLib.source_remove(this._qTimer);
      this._qTimer = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 700, () => {
        if (!this.running) {
          this._qTimer = 0;
          return GLib.SOURCE_REMOVE;
        }
        if (this._qIdx >= this._qOps.length) {
          this._qTimer = 0;
          return GLib.SOURCE_REMOVE;
        }
        this._query(this._qOps[this._qIdx]);
        this._qIdx++;
    ...
    _startRefresh() {
      ...
      this._refreshTimer = GLib.timeout_add_seconds(
        ...
        () => {
          ...
          this._refreshTick++;
          if (this._refreshTick % 5 === 0) this._querySequence();
          return GLib.SOURCE_CONTINUE;
        },
      );
    }
    ...
    _unframe(bytes) {
      ...
      while (buf.length >= 14) {
        ...
        out.push(
          new Uint8Array(buf.slice(headIdx + 6, headIdx + 6 + outerLen)),
        );
        buf = buf.slice(end);
      }
      if (buf.length > 0 && buf.length < 1000) this._reassembly = buf;
      else this._reassembly = [];
      return out;
    }
    ...
    _decode(bytes) {
      ...
      if (bytes.length < 8 + innerLen) return null;
      return { opcode, type, payload: bytes.slice(8, 8 + innerLen) };
    }

    _handle(pkt) {
      const isNotification =
        (pkt.type & 0xe0) === PACKET_TYPE.NOTIFICATION_ACK ||
        (pkt.type & 0xe0) === PACKET_TYPE.NOTIFICATION_NO_ACK;
      const isResponse =
        (pkt.type & 0xe0) === PACKET_TYPE.RESPONSE_ACK ||
      ...
      switch (pkt.opcode) {
        ...
        case OP.BATTERY_LEVEL_CHANGED:
          Log.info(
            `battery bytes (left right case): ${p.map((b) => b.toString(16).padStart(2, "0")).join(" ")}`,
          );
          if (p.length >= 3)
            this.emit("battery", {
        ...
        case OP.ANC_MODE_CHANGED:
          if (p.length >= 2) this.emit("anc", this._decodeAnc(p[0], p[1]));
          break;
        case OP.GET_TOGGLE_CONFIGS:
          if (p.length >= 1) {
            const count = p[0];
            for (let i = 1, n = 0; n < count && i + 2 < p.length; i += 3, n++) {
              if (p[i] === TOGGLE_CATEGORY.ANC_PREFERENCE)
                this.emit("anc", this._decodeAnc(p[i + 1], p[i + 2]));
              else if (p[i] === TOGGLE_CATEGORY.DUAL_CONNECTION)
                this.emit("dual", !!p[i + 1]);
            }
          }
          break;
        ...
        case OP.EQ_STATE_CHANGED:
          if (p.length >= 1) this.emit("eq-enabled", !!p[0]);
          break;
        case OP.GET_EQ_SET:
        case OP.EQ_SET_CHANGED:
          if (p.length >= 1) this.emit("eq-preset", p[0]);
          break;
        ...
        case OP.EQ_USER_BANDS_CHANGED:
          if (p.length >= 10) {
            ...
            const inRange = (arr) =>
              arr.every((v) => v >= -EQ_RANGE && v <= EQ_RANGE);
            const a = parse(0);
            const b = p.length >= 12 ? parse(2) : null;
            let bands = null;
            if (inRange(a)) bands = a;
            else if (b && inRange(b)) bands = b;
            if (bands) this.emit("eq-custom", bands);
          }
          break;
        ...
        case OP.SPATIAL_AUDIO_CHANGED:
          this.emit(
            "toggle",
            "spatial-audio",
            this._toggleVal(pkt, isResponse, p),
          );
          break;
        ...
        case OP.GET_PROFILE_VERSION:
          if (p.length >= 1)
            this.emit(
              "firmware",
              String.fromCharCode(...p).replace(/\x00/g, ""),
            );
          break;
        default:
          if (isNotification)
            Log.info(
              `unhandled opcode 0x${pkt.opcode.toString(16)} len=${p.length}`,
            );
      }
    }

    _toggleVal(pkt, isResponse, p) {
      if (isResponse) return !!(p[0] ?? 0);
      return !!(p[p.length >= 2 ? p.length - 1 : 0] ?? 0);
    }

    _battery(b) {
      if (b === 0xff) return { level: 0, charging: false, reported: false };
      return { level: b & 0x7f, charging: (b & 0x80) !== 0, reported: true };
    }

    _decodeAnc(cat, sub) {
      if (cat === 0x00 && sub === 0x00) return ANC.OFF;
      if (cat === 0x02 && sub === 0x00) return ANC.TRANSPARENCY;
      if (cat === 0x01 && sub === 0x03) return ANC.ANC;
      if (cat === 0x01 && sub === 0x01) return ANC.ADAPTIVE;
      return ANC.OFF;
    }
    ...
    setToggle(name, enabled) {
      ...
      switch (name) {
        case "bass-boost":
          this._command(OP.SET_BASS_ENHANCEMENT, [on]);
          break;
        case "volume-boost":
          this._command(OP.SET_VOLUME_BOOST, [on]);
          break;
        case "hi-res":
          this._command(OP.SET_HI_RES_MODE, [on]);
          break;
        case "game-mode":
          this._command(OP.SET_GAME_MODE, [on]);
          break;
        case "adaptive-hearing":
          this._command(OP.SET_ADAPTATION_STATUS, [on]);
          break;
        case "auto-volume":
          this._command(OP.SET_AUTO_VOLUME, [on]);
          break;
        case "case-recording":
          this._command(OP.SET_CASE_RECORDING, [on]);
          break;
        case "in-ear":
          this._command(OP.SET_IN_EAR_DETECTION, [on]);
          break;
        case "spatial-audio":
          this._command(OP.SET_SPATIAL_AUDIO, [on]);
          break;
        case "dual":
          this._command(OP.SET_DUAL_CONNECTION, [0x01, on]);
          break;
      }
    }
    ...
    destroy() {
      if (this._qTimer) {
        GLib.source_remove(this._qTimer);
        this._qTimer = 0;
      }
