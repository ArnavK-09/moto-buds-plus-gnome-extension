  ...
  class MotoBudsDevice extends GObject.Object {
    ...
    _startSocket() {
      ...
      this._socket.connectObject(
        ...
        "synced",
        () => this._set({ synced: true }),
        "battery",
        (_o, b) => {
          const merged = { ...this.state.battery };

          let leftReported = b.left.reported;
          let rightReported = b.right.reported;

          if (leftReported && rightReported) {
            if (!b.left.charging && b.right.charging) {
              rightReported = false;
            } else if (b.left.charging && !b.right.charging) {
              leftReported = false;
            }
          }

          if (leftReported) merged.left = b.left;
          else merged.left = { ...merged.left, reported: false };

          if (rightReported) merged.right = b.right;
          else merged.right = { ...merged.right, reported: false };

          if (b.case.reported) {
            if (b.case.level === 100 && merged.case.level !== 100) {
              this._case100Count = (this._case100Count || 0) + 1;
              if (this._case100Count >= 2) {
                merged.case = b.case;
              }
            } else {
              this._case100Count = 0;
              merged.case = b.case;
            }
          } else {
            this._case100Count = 0;
          ...
          this._set({ battery: merged });
          this._persist();
        },
        "anc",
        (_o, m) => this._set({ ancMode: m }),
        "dual",
        (_o, v) => this._setToggle("dual", v),
        "eq-enabled",
        (_o, v) => this._set({ eqEnabled: v }),
        "eq-preset",
        (_o, id) =>
          this._set({ eqPreset: PRESET_NAME_BY_ID[id] ?? PRESET.CUSTOM }),
        "eq-custom",
        (_o, bands) => {
        ...
        "toggle",
        (_o, name, v) => this._setToggle(name, v),
        "firmware",
        (_o, v) => this._set({ firmware: v }),
        this,
      );
    }
    ...
    _scheduleReconnect() {
      if (this._destroyed || this._reconnectId) return;
      this._reconnectAttempts++;
      if (this._reconnectAttempts > 15) return;
    ...
    _setToggle(name, value) {
      if (this.state.toggles[name] === value) return;
    ...
    setEqPreset(name) {
      ...
      if (name === PRESET.CUSTOM) this._socket?.setEqCustom(bands);
      this._persist();
    }
    ...
    _loadPersisted() {
      try {
        ...
        if (entry) {
          if (entry["eq-preset"]) this.state.eqPreset = entry["eq-preset"];
          if (Array.isArray(entry["eq-custom"]))
            this.state.eqCustom = entry["eq-custom"];
          if (entry.alias) this.state.alias = entry.alias;
          if (entry.battery) this.state.battery = entry.battery;
          if (entry.toggles) this.state.toggles = entry.toggles;
        }
      } catch (e) {
